...
export default function ChatView(props: ChatViewProps) {
  ...
  const setComposerDraftThreadReferences = useComposerDraftStore(
    (store) => store.setThreadReferences,
  );
  ...
  const restoreQueuedMessagesToComposer = (messages: ReadonlyArray<QueuedComposerMessage>) => {
    ...
    setComposerDraftThreadReferences(composerDraftTarget, [
      ...(draft?.threadReferences ?? []),
      ...messages.flatMap((message) => message.threadReferences),
    ]);
  ...
  const onSend = async (
    ...
    if (
      ...
      if (!followUpSent) {
        ...
        setComposerDraftThreadReferences(composerDraftTarget, followUpThreadReferences);
        restorePlanFollowUpComposer({
          snapshot: {
    ...
    if (failure !== null) {
      ...
      if (queuedMessage) {
        ...
        setComposerDraftThreadReferences(composerDraftTarget, composerThreadReferencesSnapshot);
        composerRef.current?.resetCursorState({
